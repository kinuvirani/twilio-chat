 ...
 */
let exclusionList = ['sortBy', 'page', 'items_per_page', 'data_per_page'];
...
module.exports = {
  ...
  /**
   * This will retrieve the data per page
   *
   * @param {object} args - The Swagger params
   *
   *
   * @returns {Number} - The items per page
   */
  getDataPerPage: function getDataPerPage(args) {
    let queryParams = this.getConvertedQueryObject(args);
    let limit = 0;

    if (!_.isNil(queryParams.data_per_page)) {
      limit = queryParams.data_per_page;
    }
